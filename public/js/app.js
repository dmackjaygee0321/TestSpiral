...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    generate: function generate(status) {
      this.load = false;
      this.newdata.data = [];
      ...
      for (var i = 0; i < num_random; i++) {
        ...
        var random_value = "";
        var random_id = "";
        var breakdown_values = {}; //for random id

        for (var y = 0; y < 5; y++) {
          var random_number = numbers.charAt(Math.floor(Math.random() * numbers.length));
          random_id += random_number;
        } //for random values


        for (var _y = 0; _y < 5; _y++) {
          var random_char = characters.charAt(Math.floor(Math.random() * characters.length));
          random_value += random_char;
        } //for breakdown values


        for (var x = 0; x < num_breakdown; x++) {
          var temp_bvalue = "";

          for (var _y2 = 0; _y2 < 5; _y2++) {
            var breakdown_char = characters.charAt(Math.floor(Math.random() * characters.length));
      ...
      this.axios.post("http://localhost:8000/api/generate", this.newdata).then(function (response) {
        if (status == 1) {
          swal({
            title: "Success!",
            text: "Success!",
    ...
    display: function display() {
      ...
      this.load = false;
      this.axios.get("http://localhost:8000/api/retrievedata", this.newdata).then(function (response) {
        _this.datas = response.data;
        setInterval(_this.displayspiral, 2000);
      });
    },
    spiraldesign: function spiraldesign() {
      ...
      var target = options.target,
          ...
          size = options.size;
      var message = target.textContent;
      var quaterWidth = target.clientWidth / 4;
      var diameter = size; // || quaterWidth;

      var messageChunks = message.split(String());
      var length = messageChunks.length;
      var offsetEnd = 1;
      var l = length + offsetEnd;
      var CIRC_MAX = 360;
      var PERC_MAX = 100;
      var ofDeg = span * CIRC_MAX / PERC_MAX;
      var ofNorth = north * CIRC_MAX / PERC_MAX;
      var segDeg = ofDeg / l;
      var fontSize = 16;
      var a = 0;
      var i = 0;
      target.setAttribute('style', "\n                        padding-left: ".concat(diameter / 2, "px;\n                        display: inline-block;\n                        width: ").concat(diameter / 2, "px;\n                        height: ").concat(diameter, "px;\n                        transform: rotate3d(0, 0, 1, -").concat(segDeg + ofNorth, "deg);\n                        border-radius: 9e9em;\n                        font-size: 1rem;\n                    "));
      ...
      while (a < ofDeg) {
        ...
        charWrapper.setAttribute('style', "\n                            display: inline-block;\n                            transform: \n                                rotate3d(0, 0, 1, ".concat(a, "deg) \n                                ").concat(spiral ? 'translateY(' + i + 'px)' : '', ";\n                            ").concat(spiral ? 'font-size:' + (fontSize - i / 10) + 'px' : '', ";\n                            position: absolute;\n                            height: ").concat(diameter / 2, "px;\n                            transform-origin: bottom left;\n                            font-size: 1em;\n                        "));
        target.appendChild(charWrapper);
      }
    },
    displayspiral: function displayspiral() {
      ...
      [
        _vm._l(_vm.datas, function(data, index) {
          return _c(
            ...
            [
              _c("label", [_vm._v("Flag: " + _vm._s(data.flag))]),
              _vm._v(" "),
              _c("p", { class: ["p" + index] }, [_vm._v(_vm._s(data.values))])
            ]
          )
        }),
        _vm._v(" "),
...
__webpack_require__(/*! C:\Users\HP\Desktop\Test Program\testspiral\resources\js\app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! C:\Users\HP\Desktop\Test Program\testspiral\resources\css\app.css */"./resources/css/app.css");
